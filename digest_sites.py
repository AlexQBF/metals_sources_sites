...
AI_SYSTEM = (
    ...
    "Правила:\n"
    "ЖЁСТКИЙ ФИЛЬТР: оставляй ТОЛЬКО золото и серебро (цены, спрос/предложение, добыча, запасы, прогнозы, "
    "отчёты, решения регуляторов/ЦБ, сделки, проекты). СТРОГО ВЫБРАСЫВАЙ платину, палладий, МПГ, медь, никель, "
    "сталь, уголь, нефть, алмазы и прочее. Если материал в основном про них — выбрасывай целиком.\n"
    "БЕРИ ТОЛЬКО НОВОСТИ И СОБЫТИЯ (что-то произошло/изменилось/заявлено/куплено/выросло/упало). "
    ...
    "Отбери значимое, не больше 8 пунктов, коротко.\n"
    "ФОРМАТ: каждый пункт —\n▪️ <b>Короткий заголовок</b>\nи 1-2 ёмких предложения. Между пунктами пустая строка. "
    "Только HTML <b>. Без вступления/заключения. Если значимого нет — верни ровно: НЕТ_НОВОСТЕЙ"
)
