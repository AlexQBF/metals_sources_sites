...
AI_SYSTEM = (
    ...
    "ОЦЕНКА ВАЖНОСТИ: по каждой новости оцени значимость для рынка золота/серебра и отрасли "
    "(масштаб события, влияние на цены, размер компании/сделки, значимость для РФ и мира). "
    ...
    "- Заголовок должен быть ПОНЯТНЫМ и информативным: из него сразу ясно, ЧТО произошло "
    "(например «Цены на золото упали ниже $4000 на фоне укрепления доллара», а не просто «Золото упало»). "
    "Не обрубай до 2-3 слов, но и не делай длиннее одной строки.\n"
    "- ССЫЛКА НА ИСТОЧНИК вшивается ВНУТРЬ заголовка, в одно самое подходящее слово: предпочтительно глагол "
    "(«упали», «купила», «выплатит», «нарастила»), а если явного глагола нет — в главное ключевое слово темы. "
    "Оформи это как <a href=\"URL\">слово</a> прямо внутри текста заголовка, остальной заголовок оставь обычным. "
    "Весь заголовок при этом обёрнут в <b>...</b>.\n"
    "- URL бери ТОЛЬКО из поля «Ссылка» соответствующего материала — НИЧЕГО НЕ ВЫДУМЫВАЙ. "
    "Если ссылки нет — оставь заголовок без ссылки. При склейке дублей используй ссылку самого содержательного источника.\n\n"
    "Пример пункта: 🔸 <b>«Полюс» <a href=\"URL\">выплатит</a> дивиденды за I квартал более 39,5 млрд рублей</b>\n"
...
def main():
    ...
    header = f"<b>🪙 AU &amp; AG — Новости рынка · {date_ru}</b>\n\n"
